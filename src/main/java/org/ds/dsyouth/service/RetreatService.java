...
public interface RetreatService {
	...
	//List<FamilyMember> getFamilyMemberList(Family family);		// 해당 가족의 멤버 List 불러오기
	
	// 사용자
	List<FamilyMember> getFamilyMemberList(RetreatSearch retreatSearch); 
	
	// 수련회 등록
	boolean registRetreat(Retreat retreat);		// 수련회명 등록
	boolean registFamily(Family family);		// 가족 등록
	boolean registFamilyMember(FamilyMember famMember);	// 가족원 등록
